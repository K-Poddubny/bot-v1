...
def format_vacancy_desc(raw: str) -> str:
    """Чистит маркдаун/HTML, распознаёт заголовки по ключам,
    всегда превращает непустые строки в «• …», а адресные простыни вырезает."""
    import re
    if not raw:
    ...
    # HTML → нормальный текст
    t = re.sub(r"(?is)<br\s*/?>", "\n", t)
    t = re.sub(r"(?is)</?p[^>]*>", "\n", t)
    t = re.sub(r"(?is)</?(ul|ol|li)[^>]*>", "\n", t)
    t = re.sub(r"(?is)</?[^>]+>", " ", t)   # остальные теги просто в пробел
    t = re.sub(r"[\t\xa0]", " ", t)

    # Маркдаун и декоративный мусор
    t = re.sub(r"\*{1,3}|`{1,3}", "", t)   # убираем ***, **, ``
    t = re.sub(r"_{2,}", "_", t)            # ____ -> _
    t = re.sub(r"^\s*[-–—]{3,}\s*$", "", t, flags=re.MULTILINE)  # «линиейки»
    t = re.sub(r"\s*[-–—]{2,}\s*", " — ", t)                      # ---, -- -> « — »
    ...
    def as_header(line: str):
        low = line.lower()
        for pat, hdr in header_rules:
    ...
    raw_lines = [strip_bullet(x) for x in t.split("\n")]
    ...
    def flush_buf():
        if not buf:
            return
        out.extend([f"• {b}" for b in buf if b])
        buf.clear()

    # Флаги для вырезки адресных простыней
    skip_addresses = False
    addr_kw = r"(ул\.|просп|шосс|пл\.|пер\.|бул\.|км\b|стр\.|дом\b|д\.|корп\.|к\.|лит\.)"

    for ln in lines:
        # Если ранее включили режим скипа адресов — пропускаем «адресные» строки
        if skip_addresses:
            # стоп-условие: пустая строка или следующий реальный заголовок
            if ln == "" or as_header(ln):
                skip_addresses = False
                if ln == "":
                    continue
            else:
                # пропускаем строки, похожие на адресные
                if ln.startswith("|") or re.fullmatch(r"[\|\-–—\s]+", ln) or re.search(addr_kw, ln, re.IGNORECASE) or ln.count(",") >= 2:
                    continue
                # если это уже не адресная строка — выходим из режима и обрабатываем дальше
                skip_addresses = False  # fallthrough
        ...
        # Включаем вырезку адресов, если встретили заголовок «ищем водител… по адрес…»
        low = ln.lower()
        if ("ищем" in low and "водител" in low and ("по адрес" in low or "адреса" in low)):
            flush_buf()
            skip_addresses = True
            # этот «заголовок» тоже не показываем
            continue
        ...
        if hdr:
            ...
            if out and out[-1] != "":
                out.append("")
            out.append(hdr)
            out.append("")  # отбивка после заголовка
            continue

        # обычная строка — пункт
        buf.append(ln)

    flush_buf()

    # Финальная уборка
    result = "\n".join([ln for i, ln in enumerate(out)
                         if not (ln == "" and (i == 0 or (i > 0 and out[i-1] == ""))) ]).strip()
