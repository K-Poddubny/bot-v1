...
def format_vacancy_desc(raw: str) -> str:
    """Очищаем маркдаун, распознаём заголовки только по ключам,
    все прочие непустые строки превращаем в «• …».
    Никаких эвристик по длине строк — пункты не пропадают."""
    import re
    if not raw:
    ...
    # Убираем «линиейки» и превращаем многотиpей в « — »
    t = re.sub(r"^\s*[-–—]{3,}\s*$", "", t, flags=re.MULTILINE)
    t = re.sub(r"\s*[-–—]{2,}\s*", " — ", t)

    # Заголовки только по ключевикам/двоеточию
    header_rules = [
    ...
    # Чистим лидирующие маркеры пунктов
    ...
    lines = [strip_bullet(x) for x in t.split("\n")]
    # Убираем лишние пустые подряд
    compact = []
    for ln in lines:
    ...
    out_lines = []
    buf = []  # текущий список пунктов

    def flush_buf():
        if not buf:
            return
        out_lines.extend([f"• {b}" for b in buf if b])
        buf.clear()

    for ln in lines:
        if ln == "":
            # пустая строка — закончить текущий набор пунктов
            flush_buf()
        ...
        if hdr:
            ...
            out_lines.append("")  # отбивка после заголовка
            continue
    ...
    flush_buf()
    # финальная уборка пустых
    res = "\n".join([ln for i, ln in enumerate(out_lines)
                      if not (ln == "" and (i == 0 or (i > 0 and out_lines[i-1] == ""))) ]).strip()
    return res or "Описание недоступно."
